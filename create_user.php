...
foreach ([
    ...
    if ($result && $result->num_rows === 0 && !$conn->query($alter)) {
        respond(false, 'The system could not prepare the faculty profession field.', 500);
    }
}
...
if ($role === 'student') {
    ...
    $sql = $conn->prepare('INSERT INTO users (role, full_name, phone, email, password, status, faculty_id, faculty_profession) VALUES (?, ?, ?, ?, ?, "Registered", ?, ?)');
    $sql->bind_param('sssssss', $role, $full_name, $phone, $email, $hashed_password, $faculty_id, $faculty_profession);
