...
foreach ([
    ...
    if ($result && $result->num_rows === 0 && !$conn->query($alter)) respond(false, 'The system could not prepare faculty qualification fields.', 500);
}
...
if ($role === 'student') {
    ...
    $sql = $conn->prepare('INSERT INTO users (role, full_name, phone, email, password, status, faculty_id, faculty_expertise, faculty_teaching_course) VALUES (?, ?, ?, ?, ?, "Registered", ?, ?, ?)');
    $sql->bind_param('ssssssss', $role, $full_name, $phone, $email, $hashed_password, $faculty_id, $faculty_expertise, $faculty_teaching_course);
    $account_id = $faculty_id;
}
if (!$sql->execute()) respond(false, 'The account could not be created. Please try again.', 500);
respond(true, 'Account created successfully. Give these credentials to the user.', 200, ['account_id' => $account_id, 'name' => $full_name, 'email' => $email, 'password' => $plain_password, 'role' => $role]);
