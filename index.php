...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['signup'])) {
        ...
        if (!hash_equals($_SESSION['csrf_token'] ?? '', $csrf)) {
            $signup_message = 'Security validation failed. Please try again.';
        } else {
                ...
                if ($existing) {
                    ...
                    if ($role === 'student') {
                        if ($student_id === '') {
                            ...
                            $signup_message = 'Please select your course.';
                        } elseif ($year_level === '') {
                            $signup_message = 'Please select your year level.';
                        } else {
                            $hashed = password_hash($password, PASSWORD_DEFAULT);
                            $sql = $conn->prepare(
                                'INSERT INTO users (role, full_name, phone, email, password, status, student_id, course, year_level) VALUES (?, ?, ?, ?, ?, "Pending", ?, ?, ?)'
                            );
                            $sql->bind_param('ssssssss', $role, $full_name, $phone, $email, $hashed, $student_id, $course, $year_level);
                        ...
                        if ($faculty_id === '') {
                            ...
                            $signup_message = 'Please select your department.';
                        } else {
                            $hashed = password_hash($password, PASSWORD_DEFAULT);
                            $sql = $conn->prepare(
                                'INSERT INTO users (role, full_name, phone, email, password, status, faculty_id, department) VALUES (?, ?, ?, ?, ?, "Pending", ?, ?)'
                            );
                            $sql->bind_param('sssssss', $role, $full_name, $phone, $email, $hashed, $faculty_id, $department);
    ...
    <div class="container">
        ...
        <div id="signupPanel" class="hidden">
            ...
            <div class="role-cards">
                <button type="button" class="role-card" id="card-student" onclick="selectRole('student')">
                    👨‍🎓<br>
                    <b>Student</b>
                </button>

                <button type="button" class="role-card" id="card-faculty" onclick="selectRole('faculty')">
                    👨‍🏫<br>
                    <b>Faculty</b>
                </button>
            </div>
            ...
            <form method="POST" id="signupForm">
                ...
                <input type="password" name="password" id="pass" placeholder="Create Password" minlength="8" required>

                <button type="button" onclick="togglePass()">Show / Hide Password</button>
